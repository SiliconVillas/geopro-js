...
export const invertAffineOrtogonalMatrix = (t: Matrix): Matrix => {
  ...
  const y = -t[3][1];
  const z = -t[3][2];
  const tx = x*t[0][0] + y*t[1][0] + z*t[2][0];
  const ty = x*t[0][1] + y*t[1][1] + z*t[2][1];
  const tz = x*t[0][2] + y*t[1][2] + z*t[2][2];

  return [
    [ t[0][0], t[0][1], t[0][2], 0 ], // col 0
    [ t[1][0], t[1][1], t[1][2], 0 ], // col 1
    [ t[2][0], t[2][1], t[2][2], 0 ], // col 2
    [      tx,      ty,      tz, 1 ], // col 3
  ];
}
